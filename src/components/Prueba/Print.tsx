import { useReactToPrint } from 'react-to-print';
import { useTranslation } from 'react-i18next';

const Print = ({ tableref }) => {

  const { t } = useTranslation(['Editor']);

  const handlePrint = useReactToPrint({
    contentRef: tableref,
    pageStyle: `
      @page {
        size: ${tableref.current?.clientWidth*1.5}px ${tableref.current?.clientHeight*2}px;
      }
        body {
              margin: 0;
              transform: scale(2);
              transform-origin: top left;
            }
              @media print {
            .no-print {
              display: none;
            }
          }
    `,
  });

  return (

    <div className="tooltip tooltip-bottom" onClick={() => handlePrint()} data-tip={t("export")}>
      <div tabIndex={0} role="button" aria-label="pdf" className="btn btn-ghost btn-circle">
        <svg className="w-6 h-6" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
          <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 17v-5h1.5a1.5 1.5 0 1 1 0 3H5m12 2v-5h2m-2 3h2M5 10V7.914a1 1 0 0 1 .293-.707l3.914-3.914A1 1 0 0 1 9.914 3H18a1 1 0 0 1 1 1v6M5 19v1a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-1M10 3v4a1 1 0 0 1-1 1H5m6 4v5h1.375A1.627 1.627 0 0 0 14 15.375v-1.75A1.627 1.627 0 0 0 12.375 12H11Z" />
        </svg>
      </div>
    </div>

  );
};

export default Print;
